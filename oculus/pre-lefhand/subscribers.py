...
# Callback functions relating to Oculus Touch button functions and head
...
def call_LH_pos_x(data):
	...
	#rospy.loginfo("LH pos y : %f", robot.goalPose.LH_pos_y)
...
def call_LH_pos_y(data):
	...
	#rospy.loginfo("LH pos z : %f", robot.goalPose.LH_pos_z)
...
def call_RH_pos_x(data):
	...
	#rospy.loginfo("RH pos y : %f", robot.goalPose.RH_pos_y)
...
def call_RH_pos_y(data):
	...
	#rospy.loginfo("RH pos z : %f", robot.goalPose.RH_pos_z)
...
def call_RH_roll(data):
	...
	# rospy.loginfo("RH Yaw is %f", robot.goalPose.RH_yaw*57.2958)

def call_RH_pitch(data):
	robot.goalPose.RH_pitch = -data.data + 90*0.0174533

def call_RH_yaw(data):
	robot.goalPose.RH_roll = -data.data
	#rospy.loginfo("RH Roll is %f", robot.goalPose.RH_roll*57.2958)